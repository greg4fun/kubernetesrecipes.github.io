---
title: "Multi-GPU PyTorch DDP on Kubernetes"
description: "Run PyTorch DistributedDataParallel across multiple GPUs on Kubernetes. torchrun, NCCL backend, pod topology, and scaling to multi-node training."
...
Run PyTorch DistributedDataParallel across multiple GPUs on Kubernetes. Without proper setup, GPU workloads on Kubernetes suffer from wasted resources, failed scheduling, or degraded inference performance.
...
```yaml
# Multi-GPU PyTorch DDP on Kubernetes — production configuration
apiVersion: v1
kind: Pod
metadata:
  name: gpu-workload
...
- Multi-GPU PyTorch DDP on Kubernetes is critical for production GPU workloads on Kubernetes
