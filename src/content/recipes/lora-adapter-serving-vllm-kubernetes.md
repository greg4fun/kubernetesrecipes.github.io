---
title: "LoRA Adapter Serving vLLM on K8s"
description: "Serve multiple LoRA adapters with a single vLLM base model on Kubernetes. Dynamic loading, per-request routing, and multi-tenant fine-tuned models."
...
Serve multiple LoRA adapters with a single vLLM base model on Kubernetes. Without proper setup, GPU workloads on Kubernetes suffer from wasted resources, failed scheduling, or degraded inference performance.
...
```yaml
# LoRA Adapter Serving vLLM on K8s — production configuration
apiVersion: v1
kind: Pod
metadata:
  name: gpu-workload
...
- LoRA Adapter Serving vLLM on K8s is critical for production GPU workloads on Kubernetes
