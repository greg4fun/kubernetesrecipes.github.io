...
flowchart TB
    subgraph GIT["📂 GIT REPOSITORY - Single Source of Truth"]
        direction TB
        APPS["📁 apps/"]
        PROD["├── production/<br/>│   ├── deployment.yaml<br/>│   ├── service.yaml<br/>│   └── kustomization.yaml"]
        STAGE["└── staging/<br/>    └── ..."]
        APPS --- PROD
        APPS --- STAGE
    end

    DEV["👨‍💻 Developer"] -->|"1️⃣ Git push"| GIT
    GIT -->|"2️⃣ Detect changes"| ARGO

    subgraph ARGO["🔶 ARGO CD"]
        direction TB
        CTRL["🎛️ Application Controller"]
        FEATURES["• Watches Git repositories<br/>• Compares desired vs actual state<br/>• Syncs automatically or on-demand<br/>• Detects and reports drift"]
        CTRL --- FEATURES
    end

    ARGO -->|"3️⃣ Apply changes"| K8S
    K8S -.->|"4️⃣ Git commit (auto-sync)"| GIT

    subgraph K8S["☸️ KUBERNETES CLUSTER"]
        RESOURCES["📦 Deployed Resources<br/>(Always matches Git state)"]
    end
```
