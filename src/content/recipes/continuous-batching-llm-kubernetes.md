---
title: "Continuous Batching LLM Inference K8s"
description: "Configure continuous batching for LLM inference on Kubernetes. vLLM and TRT-LLM batch scheduling, max-num-seqs tuning, and throughput optimization."
...
Configure continuous batching for LLM inference on Kubernetes. Without proper setup, GPU workloads on Kubernetes suffer from wasted resources, failed scheduling, or degraded inference performance.
...
```yaml
# Continuous Batching LLM Inference K8s — production configuration
apiVersion: v1
kind: Pod
metadata:
  name: gpu-workload
...
- Continuous Batching LLM Inference K8s is critical for production GPU workloads on Kubernetes
