---
title: "CloudNativePG PostgreSQL Operator K8s"
description: "Deploy PostgreSQL with CloudNativePG operator on Kubernetes. Cluster setup, affinity, replication lag monitoring, backup, and high availability configuration."
...
> 💡 **Quick Answer:** Deploy PostgreSQL with CloudNativePG operator on Kubernetes. Cluster setup, affinity, replication lag monitoring, backup, and high availability configuration.

## The Problem

Deploy PostgreSQL with CloudNativePG operator on Kubernetes. Without proper configuration, teams encounter unexpected behavior, security gaps, or performance issues in production Kubernetes clusters.
...
```yaml
# CloudNativePG PostgreSQL Operator K8s — production configuration
apiVersion: v1
kind: ConfigMap
metadata:
  name: cloudnativepg-kubernetes-postgresql-config
...
- CloudNativePG PostgreSQL Operator K8s is essential for production Kubernetes operations
