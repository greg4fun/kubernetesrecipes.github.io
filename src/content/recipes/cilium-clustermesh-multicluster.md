---
title: "Cilium ClusterMesh Multi-Cluster"
description: "Connect multiple K8s clusters with Cilium ClusterMesh. Shared services, global service discovery, and cross-cluster network policies."
...
> 💡 **Quick Answer:** Connect multiple K8s clusters with Cilium ClusterMesh. Shared services, global service discovery, and cross-cluster network policies.

## The Problem

Teams running production K8s clusters need cilium clustermesh multi-cluster for reliability, security, and operational excellence. Misconfiguration leads to outages, security gaps, or wasted resources.
...
```yaml
# Cilium ClusterMesh Multi-Cluster — production example
apiVersion: v1
kind: ConfigMap
metadata:
  name: cilium-clustermesh-multicluster-config
...
- Cilium ClusterMesh Multi-Cluster is critical for production K8s operations
